...
docker_env() {
    ...
    out "Write vhost (default php.local):" 5
    read option
    if [ -z $option ]; then
        set_dotenv VHOST "php.local"
    else
        set_dotenv VHOST $option
    fi
    out "Write database name (default php):" 5
    read option
    if [ -z $option ]; then
        set_dotenv DB_NAME "php"
    else
        set_dotenv DB_NAME $option
    fi
    out "Write database username (default php):" 5
    read option
    if [ -z $option ]; then
        set_dotenv DB_USER "php"
    else
